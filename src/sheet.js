...
function computeSnaps() {
  ...
  snapPx = {
    peek: Math.min(200, Math.round(0.28 * vh)),
    full: Math.round(0.92 * vh),
  };
}
...
export function initSheet() {
  ...
  const onPointerDown = (e) => {
    ...
    const onHandle = e.target.closest('#sheet-handle-bar') !== null;
    if (!onHandle) {
      if (scrollEl && scrollEl.scrollTop > 0) return;
      const t = e.target;
      if (t.closest('input, button, select, textarea, a, label, .route-tab, .route-step, .route-alt-label')) {
        return;
      }
    }
    dragging = true;
